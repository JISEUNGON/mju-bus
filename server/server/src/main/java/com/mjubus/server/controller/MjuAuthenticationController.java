...
public class MjuAuthenticationController {
    ...
    @PostMapping("/change-auth-status")
    ...
    public ResponseEntity<String> findAndRoleChange(@RequestBody MjuAuthInfoRequest request) {
        MjuAuthInfoResponse authInfoResponse = mjuAuthService.getAuthInfo(request);
        return ResponseEntity.ok(mjuAuthService.changeUserRole(authInfoResponse));
    }
}
