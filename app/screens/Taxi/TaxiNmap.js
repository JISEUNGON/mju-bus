...
function TaxiNmap({navigation}) {
    ...
    const reverseGc = (longitude, latitude) => {
      fetch(`https://naveropenapi.apigw.ntruss.com/map-reversegeocode/v2/gc?coords=${longitude},${latitude}&sourcecrs=epsg:4326&output=json&orders=roadaddr`, 
      {
        method: 'GET',
        headers: {
     ...
     .then((result) => {
        setStartName(result.results[0].region.area1.name + " " + result.results[0].region.area2.name + " " + result.results[0].land.name + " " + result.results[0].land.number1 + " " + result.results[0].land.number2);
        console.log(result.results[0].region.area1.name, result.results[0].region.area2.name, result.results[0].land.name, result.results[0].land.number1, result.results[0].land.number2);
      })
    ...
    return (
        <View style={styles.container}>
            ...
            <NaverMapView style={{width: '100%', height: '100%', marginVertical: 20,}}
